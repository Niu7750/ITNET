...
from ITNet_structure import ITNet_heavy
...
for s_t in [7]:
    ...
    for folder in range(4):
        for ii in range(20):
            ...
            model.TemporalConv_s.weight.requires_grad = False
            ...
            path = 'result/heavy %s/S%d-%d(%d)/%d-%.4f-%.4f/'%(mode_loss, s_s, s_t, folder, acc_epoch, acc_crop, acc_max)
            ...
            w_temporal = model.TemporalConv_t.cpu().weight.data.numpy()
            np.save(path + '/w_temporal', w_spatial)
